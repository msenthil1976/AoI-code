...
avgsim1=sim1/ru1
    ...
    sim1=0
for    i=1:ru1
    sim1=sim1+exprnd(0.5)
end
avgsim2=sim1/ru1
    ES=avgsim2 % expected service time
...
avgsim2=sim1/ru1
gamma1=avgsim2;%expected repair time
...
% end
% avgsim2=sim1/ru1
% gamma1=p1*avgsim2+(1-p1)*avgsim2^2; % expected repair time
% gamma2=p1*2*avgsim2^2+(1-p1)*2*avgsim2^4); % second order moment of repair time
...
%     sim1=0
% for    i=1:ru1
%     sim1=sim1+exprnd(0.8)
% end
% avgsim2=sim1/ru1
% % %Exponential
% gamma1=0.5;%expected repair time
% gamma2=2*gamma1^2;% second order repair time
% beta11=avgsim2; %expected service time
% beta12=2*beta11^2 % second order service time
% ES=beta11*(1+alpa*gamma1)% expected service time including breakdowns
...
beta11=p1*avgsim2+(1-p1)*0.8^2;   % expected service time
beta12=p1*2*0.8^2+(1-p1)*2*0.8^4; % second order moment of service time
ES=beta11*(1+alpa*gamma1) % expected service time including breakdowns
m1=1;
for k=0.01:0.05:0.6
...
r11=lambda(1)*beta11*alpa*gamma1
...
legend([p(2),p(3),p(4),p(5)],'M=2','M=3','M=4','M=5')
%% ======================= Part 5: Breakdown Model with respect to rho1 =======================
fprintf('Plotting Data ...\n')
for M=2:5
lambda=[];
rho=[];
...
%HyperExponential with p
%p=0.9;
% gamma1=p*1/2+(1-p)*1/(2^2);
% gamma2=p*2/(2^2)+(1-p)*2/(2^4);
% beta11=p*0.8+(1-p)*0.8^2;
% beta12=p*2*0.8^2+(1-p)*2*0.8^4;
...
legend([p(2),p(3),p(4),p(5)],'M=2','M=3','M=4','M=5')
%% ======================= Part 6: Base Line Model with respect rho1=======================
